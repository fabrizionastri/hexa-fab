...
export interface OrderItemGateway {
  ...
  ) => Promise<OrderItem | undefined> // return undefined if ID not found or server error
  getByProperty: (
    property: keyof OrderItem,
    value: any
  ) => Promise<OrderItem[] | undefined> // return undefined if server error
  getForOrder: (orderId: string) => Promise<OrderItem[] | undefined> // return undefined if server error
}
